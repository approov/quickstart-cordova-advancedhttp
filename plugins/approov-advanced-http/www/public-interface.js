module.exports = function init(exec, cookieHandler, urlUtil, helpers, globalConfigs, errorCodes, ponyfills) {
  var publicInterface = {
    ...
    approovRemoveSubstitutionHeader: approovRemoveSubstitutionHeader,
    approovAddSubstitutionQueryParam: approovAddSubstitutionQueryParam,
    approovRemoveSubstitutionQueryParam: approovRemoveSubstitutionQueryParam,
    approovAddExclusionURLRegex: approovAddExclusionURLRegex,
    approovRemoveExclusionURLRegex: approovRemoveExclusionURLRegex,
    approovPrefetch: approovPrefetch,
    approovPrecheck: approovPrecheck,
    approovGetDeviceID: approovGetDeviceID,
    approovSetDataHashInToken: approovSetDataHashInToken,
    approovFetchToken: approovFetchToken,
    approovGetMessageSignature: approovGetMessageSignature,
    approovFetchSecureString: approovFetchSecureString,
    approovFetchCustomJWT: approovFetchCustomJWT,
  ...
  function approovInitialize(config, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovInitialize", [
      config,
    ]);
  }
  ...
  function approovSetTokenHeader(header, prefix, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovSetTokenHeader", [
      header, prefix,
    ]);
  }

  function approovSetBindingHeader(header, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovSetBindingHeader", [
      header,
    ]);
  }
 
  function approovAddSubstitutionHeader(header, requiredPrefix, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovAddSubstitutionHeader", [
      header, requiredPrefix,
    ]);
  }

  function approovRemoveSubstitutionHeader(header, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovRemoveSubstitutionHeader", [
      header,
    ]);
  }

  function approovAddSubstitutionQueryParam(key, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovAddSubstitutionQueryParam", [
      key,
    ]);
  }

  function approovRemoveSubstitutionQueryParam(key, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovRemoveSubstitutionQueryParam", [
      key,
    ]);
  }

  function approovAddExclusionURLRegex(urlRegex, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovAddExclusionURLRegex", [
      urlRegex,
    ]);
  }

  function approovRemoveExclusionURLRegex(urlRegex, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovRemoveExclusionURLRegex", [
      urlRegex,
    ]);
  }

  function approovPrefetch(success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovPrefetch");
  }

  function approovPrecheck(success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovPrecheck");
  }

  function approovGetDeviceID(success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovGetDeviceID");
  }

  function approovSetDataHashInToken(data, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovSetDataHashInToken", [
      data,
    ]);
  }

  function approovFetchToken(url, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovFetchToken", [
      url,
    ]);
  }

  function approovGetMessageSignature(message, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovGetMessageSignature", [
      message,
    ]);
  }

  function approovFetchSecureString(key, newDef, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovFetchSecureString", [
      key, newDef,
    ]);
  }

  function approovFetchCustomJWT(payload, success, failure) {
    exec(success, failure, "CordovaHttpPlugin", "approovFetchCustomJWT", [
      payload,
    ]);
  }
