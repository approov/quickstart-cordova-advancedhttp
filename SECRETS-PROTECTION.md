...
```Javascript
cordova.plugin.http.approovAddSubstitutionHeader("your-header", "");
```
...
You can see a [worked example](https://github.com/approov/quickstart-cordova-advancedhttp/blob/master/SHAPES-EXAMPLE.md#shapes-app-with-secrets-protection) for the Shapes app.

If the secret value is provided as a parameter in a URL query string with the name `your-param` then it is necessary to notify the plugin that the query parameter is subject to substitution. You do this by making the call once, after initialization:

```Javascript
cordova.plugin.http.approovAddSubstitutionQueryParam("your-param");
```
...
In this case it is possible to make an explicit call at runtime to obtain the secret value, for apps passing attestation. Here is an example for using the required method:

```Javascript
cordova.plugin.http.approovFetchSecureString("your-secret-name", null,
    (secret) => {
        // success function - use `secret` as required, but never cache or store its value - note `secret`
        // will be null if the provided name is not defined
    },
    (err) => {
        if (err.type == "rejection")
            // failure due to the attestation being rejected, err.rejectionARC and err.rejectionReasons may
            // be used to present information to the user (note err.rejectionReasons is only available if the
...
```Javascript
cordova.plugin.http.setDevKey("uDW9FuLVpL1_4zo1");
```
...
cordova.plugin.http.approovPrecheck(
    ...
    (err) => {
        if (err.type == "rejection")
            // failure due to the attestation being rejected, err.rejectionARC and err.rejectionReasons
            // may be used to present information to the user (note err.rejectionReasons is only available
